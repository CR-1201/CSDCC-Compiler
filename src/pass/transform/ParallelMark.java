...
public class ParallelMark implements Pass {
        ...
        for (Function function : irModule.getFunctionsArray()) {
            ...
            for (BasicBlock block : function.getBasicBlocksArray()) {
                if (!knownBlocks.contains(block) && block.isLoopHeader()) {
                    ...
                    new BasicMark().mark(block.getLoop());
                    new AdvancedMark().mark(block.getLoop());
    ...
    private static class AdvancedMark {

        /*
        TODO <1> 数组写操作 -> 数组下标必须包含直接并行循环变量 ( a[i][...][...] = ... )
        TODO <2> 数组读操作 -> <1> 读数组与写数组一致 -> 必须与写数组在直接循环变量上一致 ( a[i][...] = a[i][...] + ... )
        TODO                 <2> 读数组与写数组不一致 -> 无限制 ( a[i] = b[...] + ... )
        TODO <3> 非数组写操作 -> <1> 不能有对并行循环变量的写操作 [ i = i + 1 ]
        TODO                   <2> 并行循环外被使用 -> 计算过程不能包含并行循环变量 [ a = i + ... ]
        TODO                   <3> 并行循环外不被使用 -> 无限制 (  t = i + ... )
        TODO <4> 非数组读操作 -> 无限制 ( ... = t )
         */
    ...
    private static class BasicMark {  // FIXME 有bug
